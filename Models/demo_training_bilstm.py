...
def fullTrain():
    ...
    #         if i > 0 and i % 4 == 0:
            ...
            if loss_test > loss_train and accuracy_test > max_accuracy and min_loss > loss_test:
                saver.save(sess, '../saved_model/bilstm_full_train_best')
                min_loss = loss_test
            ...
            if i % 10 == 9:
                saver.save(sess, '../saved_model/bilstm_full_train', global_step = i + 1)

        results.to_csv('../saved_model/results_%s_bilstm.csv' % (dim_lstm), index = False)
    ...
    with tf.Session() as sess:
        ...
        for i in range(batch_iterations):
            ...
            if i > 0 and (i % 10) == 9:
                ...
                print('step: %s, test loss: %s, test accuracy: %s' % (i, loss_test, accuracy_test))
                if loss_test > loss_train and accuracy_test > max_accuracy and min_loss > loss_test:
                    saver.save(sess, '../saved_model/bilstm_batch_train_best')
                    min_loss = loss_test
                    max_accuracy = accuracy_test
                if i % 10 == 9:
                    saver.save(sess, '../saved_model/bilstm_batch_train', global_step = i + 1)
