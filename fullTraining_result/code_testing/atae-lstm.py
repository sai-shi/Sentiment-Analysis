...
def fullTrain():
       ...
       plt.ylabel('Error')
       plt.title('atae-lstm Learning Curve')
       ax.annotate('batch=' + str(0) + ',iter=' + str(full_iterations) + ',learning_rate=' + str(
           learning_rate) + ',dim_lstm=' + str(dim_lstm) + ',dim_word=' + ',dim_ae=' + str(
           dim_aspect_embedding),
       ...
       df.to_csv(r"atae-lstm_performance.csv", mode='a', header=True, index=False)
...
def batchTrain():
       ...
       ax.annotate('batch=' + str(batch_size) + ',iter=' + str(batch_iterations) + ',learning_rate=' + str(
           learning_rate) + ',dim_lstm=' + str(dim_lstm) + ',dim_word=' + str(dim_word) + ',dim_ae=' + str(
           dim_aspect_embedding),
                   xy=(0.5, 0), xytext=(0, 10),
                   xycoords=('axes fraction', 'figure fraction'),
                   textcoords='offset points',
       ...
       res = {'Train_accuracy': [acc_train], 'Test_accuracy': [acc_test], 'batch_size': [batch_size],
              ...
              'dim_word': [dim_word], 'dim_ae': [dim_aspect_embedding]}
       df = pd.DataFrame(data=res)
       df.to_csv(r"atae-lstm_performance.csv", mode='a', header=True, index=False)
